...
class UserFriendshipTest < ActiveSupport::TestCase
	...
	test "that creating a friendship works without raising an exception" do
  		assert_nothing_raised do 
 			UserFriendship.create user: users(:john), friend: users(:mike)
 		end
 	end	

 	test "that creating a friendship based on user id and friend id works" do 
 		UserFriendship.create user_id: users(:john).id, friend_id: users(:mike).id
 		assert users(:john).pending_friends.include?(users(:mike))  
 	end	

 	context "a new instance" do
 		setup do
 			@user_friendship = UserFriendship.new user: users(:john), friend: users(:mike)
 		end
 	...
 	context "#send_request_email" do 
 		setup do 
 			@user_friendship = UserFriendship.create user: users(:john), friend: users(:mike)
 		end
 	...
 	context "#mutual_friendship" do
 		setup do
 	        UserFriendship.request users(:john), users(:jane)
 	        @friendship1 = users(:john).user_friendships.where(friend_id: users(:jane).id).first
 	    	@friendship2 = users(:jane).user_friendships.where(friend_id: users(:john).id).first 
 	    end
 	...
 	context "#accept!" do
 		setup do
 			@user_friendship = UserFriendship.request users(:john), users(:mike)
 		end
 	...
 	context ".request" do
 		should "create two user friendships" do
 			assert_difference 'UserFriendship.count', 2 do
 				UserFriendship.request(users(:john), users(:mike))
 			end
 		end
